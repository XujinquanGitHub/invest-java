...
public class CookiesUtil {
    ...
    public static String saveCookie(CookiesModel cookie) {
        File file = new File("D:\\Projects\\Document\\cookies.json"); if (file.exists()) {
            String s = FileUtil.readString(file, CharsetUtil.UTF_8);
            List<CookiesModel> listArr = JSON.parseArray(s, CookiesModel.class); List<CookiesModel> collect =
                    listArr.stream().filter(u -> u.getWebName().equalsIgnoreCase(cookie.getWebName()))
                            .collect(Collectors.toList()); if (CollUtil.isEmpty(collect)) {
                listArr.add(cookie);
            } else {
                collect.get(0).setCookies(cookie.getCookies());
            } FileUtil.writeString(JSON.toJSONString(listArr), file, CharsetUtil.UTF_8);
        } else {
            List<CookiesModel> listArr = new ArrayList<>(); listArr.add(cookie);
            FileUtil.writeString(JSON.toJSONString(listArr), file, CharsetUtil.UTF_8);
        } return "succ";
    }
    ...
    public static String getCookie(String webName) {
        try {
            File file = new File("D:\\Projects\\Document\\cookies.json"); if (file.exists()) {
                String s = FileUtil.readString(file, CharsetUtil.UTF_8);
                List<CookiesModel> listArr = JSON.parseArray(s, CookiesModel.class); List<CookiesModel> collect =
                        listArr.stream().filter(u -> u.getWebName().equalsIgnoreCase(webName))
                                .collect(Collectors.toList()); if (CollUtil.isEmpty(collect)) {
                    return "";
                } else {
